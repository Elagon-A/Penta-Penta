...
- The retainer Items for Sale window has a compact native-price scan button that snapshots its watched 5/5 listings, searches those distinct items sequentially at a nearby marketboard, verifies each returned item ID, and populates Current/Proposed pricing data without selecting retainer rows or requiring the optional materia-shopping overlay.
