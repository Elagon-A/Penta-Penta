...
- The earlier retainer-adjacent marketboard-search button is hidden while a retainer-native row workflow is validated, preventing it from being confused with the separate optional materia-shopping overlay.
- A read-only two-row calibration beside the retainer sale list records the current client's real row-selection and Compare Prices event sequence, selected index, live renderer index, and source node for rows 1 and 3 before retainer-native automation is enabled.
- A manually armed one-row safety test locates the calibrated live row renderer, invokes only its owning list's single SelectItem dispatch, and verifies the Adjust Price item name without requesting market data or changing a price.
...
- Captured retainer rows with a valid proposed price different from their current price are highlighted in red; unchanged and unavailable proposals retain the normal background.
